...
def andrews_films_and_leads
  ...
  execute(<<-SQL)
    ...
    JOIN castings AS lead_cast
      ON lead_cast.movie_id = movies.id
    JOIN actors AS lead_act
      ON lead_act.id = lead_cast.actor_id
    WHERE
      lead_cast.ord = 1 AND actors.name = 'Julie Andrews';
